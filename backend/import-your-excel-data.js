...
async function importYourExcelData() {
  try {
    console.log('🚀 Starting import of your Excel files...');
    ...
    try {
      ...
      const medicalResult = await importFile(db, '/Users/kashyapanand/Desktop/DBcopy/processed_files_recursive/medical.xlsx', 'MEDICAL', importStats);
      
      // Import Dental Colleges
      console.log('\n🦷 Importing Dental Colleges...');
      const dentalResult = await importFile(db, '/Users/kashyapanand/Desktop/DBcopy/processed_files_recursive/dental.xlsx', 'DENTAL', importStats);
      
      // Import DNB Colleges
      console.log('\n🏥 Importing DNB Colleges...');
      const dnbResult = await importFile(db, '/Users/kashyapanand/Desktop/DBcopy/processed_files_recursive/dnb.xlsx', 'DNB', importStats);
...
async function importFile(db, filePath, collegeType, importStats) {
  try {
    ...
    // Process each row
    const colleges = new Map(); // Use Map to deduplicate colleges
    const programs = [];
    ...
    for (let i = 1; i < jsonData.length; i++) {
      ...
      try {
        ...
        const collegeKey = `${cleanedCollege.name.toLowerCase()}_${cleanedCollege.state.toLowerCase()}`;
        
        // Add to colleges map (deduplication)
        if (!colleges.has(collegeKey)) {
          colleges.set(collegeKey, cleanedCollege);
        }
        
        // Add program data
        if (collegeData.course) {
          programs.push({
            college_name: cleanedCollege.name,
    ...
    console.log(`🏫 Unique colleges found: ${colleges.size}`);
    console.log(`📚 Programs found: ${programs.length}`);
    
    // Import colleges
    const collegeIdMap = new Map(); // Map from college name to database ID
    ...
    for (const [key, college] of colleges) {
      try {
        ...
        collegeIdMap.set(college.name, result.lastID);
    ...
    for (const program of programs) {
      try {
        const collegeId = collegeIdMap.get(program.college_name);
        if (!collegeId) {
          console.log(`⚠️  College not found for program: ${program.college_name}`);
          importStats.programs.errors++;
          continue;
        }
        
        // Determine program details based on course name
        const programDetails = getProgramDetails(program.course, program.college_type);
    ...
    return { colleges: colleges.size, programs: programs.length };
...
module.exports = { importYourExcelData };
