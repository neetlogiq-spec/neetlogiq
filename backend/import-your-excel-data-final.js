...
async function importYourExcelDataFinal() {
  try {
    console.log('🚀 Starting final import with fallback strategies...');
    ...
    try {
      ...
      const medicalResult = await importFileFinal(db, '/Users/kashyapanand/Desktop/DBcopy/processed_files_recursive/medical.xlsx', 'MEDICAL', importStats);
      
      // Import Dental Colleges
      console.log('\n🦷 Importing Dental Colleges...');
      const dentalResult = await importFileFinal(db, '/Users/kashyapanand/Desktop/DBcopy/processed_files_recursive/dental.xlsx', 'DENTAL', importStats);
      
      // Import DNB Colleges
      console.log('\n🏥 Importing DNB Colleges...');
      const dnbResult = await importFileFinal(db, '/Users/kashyapanand/Desktop/DBcopy/processed_files_recursive/dnb.xlsx', 'DNB', importStats);
...
async function importFileFinal(db, filePath, collegeType, importStats) {
  try {
    ...
    // Process each row and group by college with fallback strategies
    ...
    for (let i = 1; i < jsonData.length; i++) {
      ...
      try {
        ...
        // Create unique key with fallback strategies
        let collegeKey;
        if (cleanedCollege.address && cleanedCollege.address !== 'Unknown') {
          // Primary strategy: college name + address
          collegeKey = `${cleanedCollege.name.toLowerCase()}_${cleanedCollege.address.toLowerCase()}`;
        } else if (cleanedCollege.university) {
          // Fallback 1: college name + state + university
          collegeKey = `${cleanedCollege.name.toLowerCase()}_${cleanedCollege.state.toLowerCase()}_${cleanedCollege.university.toLowerCase()}`;
        } else if (cleanedCollege.management_type) {
          // Fallback 2: college name + state + management type
          collegeKey = `${cleanedCollege.name.toLowerCase()}_${cleanedCollege.state.toLowerCase()}_${cleanedCollege.management_type.toLowerCase()}`;
        } else {
          // Final fallback: college name + state
          collegeKey = `${cleanedCollege.name.toLowerCase()}_${cleanedCollege.state.toLowerCase()}`;
        }
    ...
    for (const [key, college] of collegeMap) {
      if (count < 5) {
        const identifier = college.address && college.address !== 'Unknown' 
          ? college.address 
          : `${college.state}${college.university ? ' - ' + college.university : ''}${college.management_type ? ' (' + college.management_type + ')' : ''}`;
        console.log(`    ${college.name} - ${identifier}`);
        count++;
      } else break;
    }
...
if (require.main === module) {
  importYourExcelDataFinal();
}

module.exports = { importYourExcelDataFinal };
