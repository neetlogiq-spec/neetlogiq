...
class UnifiedSearchEngine {
  ...
  async executeAISearch(query, options) {
    ...
    try {
      const response = await fetch(`http://localhost:8787/api/ai-search?q=${encodeURIComponent(query)}&type=${contentType}`);
      if (response.ok) {
  ...
  async executeRegexSearch(query, options) {
    ...
    try {
      const response = await fetch(`http://localhost:8787/api/colleges/regex-search?pattern=${encodeURIComponent(query)}&limit=${options.limit || 50}`);
      if (response.ok) {
        const data = await response.json();
        return {
          results: data.data || [],
