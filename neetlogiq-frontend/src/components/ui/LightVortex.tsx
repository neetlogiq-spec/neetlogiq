...
export const LightVortex = (props: LightVortexProps) => {
  ...
  const drawParticle = (
    ...
    // Optimized colors for light mode - darker, more saturated colors for better visibility
    ctx.strokeStyle = `hsla(${hue}, 85%, 45%, ${fadeInOut(life, ttl) * 0.7})`;
    ctx.beginPath();
    ctx.moveTo(x, y);
    ctx.lineTo(x2, y2);
    ctx.stroke();
  ...
  const renderGlow = (
    ...
    // Reduced glow intensity for light mode
    ctx.save();
